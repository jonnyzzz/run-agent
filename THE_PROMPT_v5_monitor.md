...
1. List active runs under <RUNS_DIR>/ and verify which agents are still running (PID checks via pid.txt; if missing, read EXIT_CODE= from cwd.txt to mark finished). If shell access is restricted, do this via MCP Steroid file reads.
